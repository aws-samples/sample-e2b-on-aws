...
# -------------------------------------------------------------------------------------------------
# Upload envs from disk to AWS S3
# -------------------------------------------------------------------------------------------------
...
# Third argument is AWS region (optional, defaults to us-east-1)
AWS_REGION=${3:-us-east-1}

# Fourth argument is template id that you can specify to upload a specific env
TEMPLATE_ID=${4:-}
# -------------------------------------------------------------------------------------------------

echo "Uploading envs from ${TARGET_DIR_NAME} to AWS S3"
...
for template_id in $(ls ${TARGET_DIR_NAME}); do
  ...
  # Upload env to S3, copy only "memfile", "rootfs.ext4" and "snapfile" from the dir
  ...
  BUCKET_MEMFILE_PATH="s3://${TEMPLATE_BUCKET_NAME}/${BUILD_ID}/memfile"
  BUCKET_ROOTFS_EXT4_PATH="s3://${TEMPLATE_BUCKET_NAME}/${BUILD_ID}/rootfs.ext4"
  BUCKET_SNAPFILE_PATH="s3://${TEMPLATE_BUCKET_NAME}/${BUILD_ID}/snapfile"

  # Upload the files with AWS CLI
