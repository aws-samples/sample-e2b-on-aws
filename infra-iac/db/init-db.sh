...
# Step 2: Seed the initial user, team and team API key.
#
# No access token: upstream migration 20260823120000 drops the access_tokens
# table and its generator outright ("nothing issues, validates, or purges them
# anymore"), and the seeder no longer prints one. The team API key is now the
# only credential this deployment issues, and it is what
# tools/legacy/create_template.sh authenticates with.
#
# This delegates to upstream's own seeder instead of the flattened
# .seed-db.sql we used to run. Two reasons:
#
#   1. The plaintext columns it wrote are gone. team_api_keys now stores only
#      api_key_hash plus prefix, length and mask columns, all NOT NULL. The hash
#      is computed by packages/shared/pkg/keys, so reproducing it in SQL would
#      mean re-implementing (and then re-verifying on every sync) key derivation
...
for VAR_NAME in teamId teamApiKey; do
    if [ -z "${!VAR_NAME}" ]; then
...
# accessToken is deleted rather than left behind: a stale value from a previous
# deployment would look like a usable credential to anything reading this file,
# and nothing validates it any more.
jq --arg teamId "$teamId" \
   --arg teamApiKey "$teamApiKey" \
   '. + {teamId: $teamId, teamApiKey: $teamApiKey} | del(.accessToken)' \
